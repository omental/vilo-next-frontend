...
export default function InvoiceDetailPage(){
  ...
  if(error) return <div className="vilo-state-block"><p className="vilo-state vilo-state--error">{error}</p></div>;
  ...
  return <section className="dashboard-page-stack"><div className="dashboard-page-heading"><h1>Invoice {inv.invoice_number}</h1></div>
    ...
    <article className="dashboard-card vilo-detail-card"><div className="dashboard-card__header"><h2>Summary</h2></div><div className="vilo-detail-grid"><p><strong>Status:</strong> <span className={`vilo-badge vilo-badge--${inv.status}`}>{inv.status}</span></p><p><strong>Client:</strong> #{inv.client_id}</p><p><strong>Case:</strong> {inv.case_id?`#${inv.case_id}`:'-'}</p><p><strong>Total:</strong> {inv.total}</p><p><strong>Paid:</strong> {inv.paid_amount}</p><p><strong>Balance Due:</strong> {inv.balance_due}</p></div></article>
